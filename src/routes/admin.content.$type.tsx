...
const TABLE_MAP: Record<string, { table: string; title: string; fields: { key: string; label: string; type?: "text" | "textarea" | "number" | "boolean" | "json" | "tags"; }[] }> = {
  packages: {
    table: "packages",
    title: "Packages",
    fields: [
      { key: "slug", label: "Slug" }, { key: "name", label: "Name" }, { key: "tagline", label: "Tagline" },
  ...
  services: {
    ...
    fields: [
      ...
      { key: "icon", label: "Icon (lucide name)" }, { key: "position", label: "Order", type: "number" },
      { key: "published", label: "Published", type: "boolean" },
    ],
  },
  addons: {
    table: "addons", title: "Add-ons",
    fields: [
      { key: "name", label: "Name" }, { key: "description", label: "Description", type: "textarea" },
      { key: "price", label: "Price", type: "number" }, { key: "position", label: "Order", type: "number" },
      { key: "published", label: "Published", type: "boolean" },
    ],
  },
  portfolio: {
    table: "portfolio_items", title: "Portfolio",
    fields: [
      { key: "title", label: "Title" }, { key: "description", label: "Description", type: "textarea" },
      { key: "image_url", label: "Image URL" }, { key: "url", label: "Project URL" },
      { key: "tags", label: "Tags (comma-separated)", type: "tags" },
      { key: "position", label: "Order", type: "number" }, { key: "published", label: "Published", type: "boolean" },
    ],
  },
  partners: {
    table: "partners", title: "Partners",
    fields: [
      { key: "name", label: "Name" }, { key: "logo_url", label: "Logo URL" }, { key: "url", label: "Website" },
      { key: "position", label: "Order", type: "number" }, { key: "published", label: "Published", type: "boolean" },
    ],
  },
  team: {
    table: "team_members", title: "Team",
    fields: [
      { key: "name", label: "Name" }, { key: "role", label: "Role" },
      { key: "bio", label: "Bio", type: "textarea" }, { key: "image_url", label: "Image URL" },
      { key: "position", label: "Order", type: "number" }, { key: "published", label: "Published", type: "boolean" },
    ],
  },
};
...
function ContentPage() {
  ...
  async function load() {
    if (!cfg) return;
    const { data } = await supabase.from(cfg.table as any).select("*").order("position", { ascending: true });
    setRows((data ?? []) as any[]);
  }
  useEffect(() => { load(); }, [type]);
  ...
  async function remove(id: string) {
    ...
    try { await del({ data: { table: cfg.table as any, id } }); toast.success("Deleted"); await load(); }
    catch (e: any) { toast.error(e.message); }
  }
            ...
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td className="px-4 py-3 text-right space-x-3">
                  <button onClick={() => setEditing(r)} className="text-brand text-sm">Edit</button>
                  <button onClick={() => remove(r.id)} className="text-red-600 text-sm">Delete</button>
                </td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-ink/40">Empty. Click "+ New".</td></tr>}
          </tbody>
        </table>
      </div>

      {editing && (
        <div className="fixed inset-0 z-50 bg-ink/40 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
            ...
            {cfg.fields.map((f) => (
              <label key={f.key} className="flex flex-col gap-1 text-sm">
                ...
                {f.type === "textarea" ? (
                  ...
                  <input value={Array.isArray(editing[f.key]) ? editing[f.key].join(", ") : (editing[f.key] ?? "")}
