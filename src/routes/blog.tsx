  ...
  head: () => ({
    meta: [
      { title: "Blog — OKIKE" },
      {
        name: "description",
        content:
          "Insights, tutorials, and stories from OKIKE.",
      },
    ...
    <SiteLayout>
      ...
      <section className="py-24 md:py-32 px-6 bg-secondary border-y border-ink/5">
        <div className="max-w-4xl mx-auto text-center flex flex-col items-center gap-8">
          ...
          <p className="text-ink/65 max-w-[48ch] text-pretty">
...
function BlogGrid({ posts, formatDate }: { posts: BlogPost[]; formatDate: (date: string | null) => string }) {
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      {posts.map((post) => {
        const card = (
          <div className="group h-full bg-card rounded-2xl ring-1 ring-ink/5 overflow-hidden flex flex-col hover:ring-brand/30 hover:-translate-y-1 transition-all">
            ...
            <div className="p-6 flex flex-col gap-4 flex-1">
              ...
              <div className="flex flex-col gap-2 flex-1">
                ...
                {post.excerpt && (
                  <p className="text-sm text-ink/60 line-clamp-3">{post.excerpt}</p>
                )}
              </div>
              {post.tags?.length > 0 && (
