...
const KEYS: { key: string; label: string; fields: { name: string; label: string; type?: "text" | "textarea" }[] }[] = [
  {
  ...
  },
  {
    ...
    fields: [{ name: "email", label: "Email" }, { name: "phone", label: "Phone" }, { name: "address", label: "Address" }],
  },
];
            ...
            <label key={f.name} className="flex flex-col gap-1 text-sm">
              ...
              {f.type === "textarea" ? (
                <textarea rows={3}
                  ...
                  onChange={(e) => setSettings({ ...settings, [g.key]: { ...(settings[g.key] ?? {}), [f.name]: e.target.value } })}
