...
function AboutPage() {
  return (
    <SiteLayout>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-12 gap-12 items-start">
          <aside className="md:col-span-5 md:sticky md:top-28 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="text-xs text-ink/50 uppercase tracking-widest">Letter from the founder</div>
            </div>
          </aside>
          <article className="md:col-span-7 flex flex-col gap-6 text-lg text-ink/80 leading-relaxed">
            ...
            <blockquote className="border-l-2 border-brand pl-6 my-2 text-2xl font-medium text-ink leading-snug">
              We're small on purpose. Every project gets a senior builder. Every student gets a real mentor.
            </blockquote>
            <p>
              Quality is not a marketing word — it's the only thing that compounds.
            </p>
      ...
      <section className="py-24 bg-secondary border-y border-ink/10 px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-12 gap-12">
          <header className="md:col-span-4">
            ...
            <h2 className="text-3xl md:text-4xl font-medium text-balance">Three principles that guide every line of code and every lesson.</h2>
          </header>
      ...
      <section className="px-6 py-24">
        <div className="max-w-7xl mx-auto">
          <div className="text-xs font-semibold tracking-widest uppercase text-brand mb-4">Two practices, one mission</div>
          <h2 className="text-3xl md:text-4xl font-medium mb-12 max-w-[28ch] text-balance">A software house and an academy under one roof.</h2>
...
function Principle({ n, t, d }: { n: string; t: string; d: string }) {
  return (
    <div className="grid grid-cols-12 gap-6 py-8 first:pt-0 last:pb-0 items-baseline">
      <div className="col-span-2 md:col-span-1 text-sm text-brand font-semibold tabular-nums">{n}</div>
...
function Pillar({
  ...
  return (
    <div className="bg-surface p-8 md:p-12 flex flex-col gap-4">
      ...
      <Link to={cta.to} className="mt-4 self-start text-sm font-medium text-brand hover:underline underline-offset-4">
        {cta.label} →
      </Link>
    </div>
  );
